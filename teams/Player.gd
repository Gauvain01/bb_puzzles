...
func set_player_action_menu_signals():
	...
	ui_component.action_menu_component.get_end_action_signal().connect(on_end_action, CONNECT_ONE_SHOT)
	
	if is_blitz_available:
		ui_component.action_menu_component.get_blitz_signal().connect(on_blitz_action, CONNECT_ONE_SHOT)
	else:
