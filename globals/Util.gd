...
static func isInsideSquare(A:Vector2, B:Vector2, C:Vector2, D:Vector2, P:Vector2) -> bool:
	if triangleAndSquareAreaComparison(A,B,P)>0 || triangleAndSquareAreaComparison(B,C,P)>0|| triangleAndSquareAreaComparison(C,D,P)>0 || triangleAndSquareAreaComparison(D,A,P)>0.: 
		return false;
	else:
		return true;
