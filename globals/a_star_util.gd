...
static func calculate_diagonal_heuristic(curr_cell:Vector2, goal:Vector2, step_len:float = 1, diagonal_len:float =1):
...
static func calculate_a_star_algo(start_coord:Vector2, goal_coord:Vector2, max_length, grid_map_allowance:Dictionary):
	
	if typeof(grid_map_allowance.values()[0]) != TYPE_BOOL:
		assert(false, "TYPE:ERROR grid_map_allowance does not contain TYPE_BOOL as values, the map needs to represent Dictionary[Vector2:bool]")
	if typeof(grid_map_allowance.keys()[0]) != TYPE_VECTOR2:
		assert(false, "TYPE:ERROR grid_map_allowance does not contain TYPE_VECTOR2 as keys, the map needs to represent Dictionary[Vector2:bool]")
