...
func request_to_place_on_field(board_piece:Player, field_square):
	if field_square.occupied != null and get_parent().game_state == GAME_STATE.SETUP:
		if field_square.player_team == "opponent" and  game_controller.opponent_placement_allowed == false:
			LogController.add_text("ERROR: you can't place your player on the opponents half of the pitch")
