...
class FieldSquare extends Area2D:
	...
	func _draw():
		draw_rect(Rect2(Vector2(-35/2,-35/2), Vector2(self.size.x-1,self.size.y-1)), self.color)
	...
	func _init(newPosition, player_team, zone, color, size, gridCoordinate:Vector2):
		...
		self.rollDisplayComponent = preload("res://components/component_scenes/roll_display_component.tscn").instantiate()
		self.select_component = preload("res://components/component_scenes/select_component.tscn").instantiate()
